    ...
    if (this.reloadFlg) {
        ...
        switch (no) {
            case 0:
                ...
                this.createModel();
                this.models[0].loadObject(gl, LAppDefine.MODEL_DEFAULT);
                break;
            case 1:
                this.releaseModel(0, gl);
                this.createModel();
                this.models[0].loadObject(gl, LAppDefine.MODEL_DEFAULT_XMAS);
                break;
            default:
                break;
        }
...
LAppLive2DManager.prototype.tapEvent = function (x, y) {
    ...
    for (var i = 0; i < this.models.length; i++) {
        this.models[i].startRandomMotion("thanking", LAppDefine.PRIORITY_NORMAL);
